...
#if defined(NDEBUG)
# error "Kacaya cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6ac4847e5848de53e5b4189befde217851db48f952272d4ec154f6cfcb2287b2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kacaya: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kacaya Signed Message:\n";
...
{
    // Kacaya: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Kacaya: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Kacaya: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Kacaya: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kacaya: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kacaya: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6ac4847e5848de53e5b4189befde217851db48f952272d4ec154f6cfcb2287b2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Kacaya Future Foundation";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("192284710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1489124300;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1093041;
        ...
        assert(block.hashMerkleRoot == uint256("0x8fdd4707507de95e533b0ddd1ef70ea4d9de73a791ad6fffc382bb09dcbe3d95"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Kacaya: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KacayaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KacayaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KacayaMiner(CWallet *pwallet)
{
    printf("KacayaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kacaya-miner");
    ...
    try { loop {
        ...
        printf("Running KacayaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KacayaMiner, pwallet));
}
